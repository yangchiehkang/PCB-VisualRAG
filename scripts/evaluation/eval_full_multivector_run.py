...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Full Multi-vector run file from project config.")
    ...
    if args.subset_only:
        ...
        metrics_file = PROJECT_ROOT / cfg["outputs"]["small_metrics_file"]
    else:
        run_file = PROJECT_ROOT / cfg["outputs"]["full_run_file"]
        metrics_file = PROJECT_ROOT / cfg["outputs"]["full_metrics_file"]
    ...
    metrics_file.write_text(json.dumps(output_obj, indent=2, ensure_ascii=False), encoding="utf-8")
