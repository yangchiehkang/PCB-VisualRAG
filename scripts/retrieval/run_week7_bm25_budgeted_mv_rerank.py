...
def select_top_m_tokens_by_norm(page_emb, m):
    if page_emb.shape[0] <= m:
        return page_emb
...
def rerank_for_config(config):
    ...
    score_debug_path = RESULT_DIR / "bm25_budgetmv_N{}_M{}_{}_scores.csv".format(n, m, compression)
    validation_path = RESULT_DIR / "bm25_budgetmv_N{}_M{}_{}_validation.csv".format(n, m, compression)
    ...
    total_scored = 0
    total_written = 0
    total_page_tokens_before = 0
    total_page_tokens_after = 0
    ...
    for qid in sorted(by_query.keys()):
        ...
        for item in candidates:
            ...
            try:
                page_emb_full = load_embedding(PAGE_EMB_DIR, pid)
            except FileNotFoundError:
                missing_page_embeddings.append(pid)
                continue

            before_tokens = page_emb_full.shape[0]
            page_emb_budgeted = select_top_m_tokens_by_norm(page_emb_full, m)
            after_tokens = page_emb_budgeted.shape[0]

            total_page_tokens_before += before_tokens
            total_page_tokens_after += after_tokens

            fine_score = late_interaction_score(query_emb, page_emb_budgeted)
            ...
            scored_items.append(
                {
                    ...
                    "page_tokens_before": before_tokens,
                    "page_tokens_after": after_tokens,
                }
            )
        ...
        for rank, item in enumerate(write_items, start=1):
            run_rows.append(
                [
                    "bm25_budgetmv_N{}_M{}_{}".format(n, m, compression),
                    item["query_id"],
                    item["page_id"],
                    rank,
        ...
        for rank, item in enumerate(scored_items, start=1):
            score_debug_rows.append(
                [
                    ...
                    rank,
                    "{:.8f}".format(item["fine_score"]),
                    item["page_tokens_before"],
                    item["page_tokens_after"],
                ]
            )
    ...
    avg_page_tokens_before = total_page_tokens_before / total_scored if total_scored else 0.0
    avg_page_tokens_after = total_page_tokens_after / total_scored if total_scored else 0.0
    token_keep_ratio = avg_page_tokens_after / avg_page_tokens_before if avg_page_tokens_before else 0.0
    ...
    summary = {
        ...
        "avg_page_tokens_before": avg_page_tokens_before,
        "avg_page_tokens_after": avg_page_tokens_after,
...
def write_summary_csv(summaries):
    path = RESULT_DIR / "bm25_budgetmv_rerank_summary.csv"
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "avg_page_tokens_before",
                "avg_page_tokens_after",
                "token_keep_ratio",
                "rerank_time_seconds",
        ...
        for s in summaries:
            writer.writerow(
                [
                    ...
                    "{:.4f}".format(s["avg_page_tokens_before"]),
                    "{:.4f}".format(s["avg_page_tokens_after"]),
                    "{:.6f}".format(s["token_keep_ratio"]),
...
def write_summary_json(summaries):
    path = RESULT_DIR / "bm25_budgetmv_rerank_summary.json"
...
def main():
    ...
    print("[Week7-Day2] Starting BM25 + Budgeted MV reranking...")

    summaries = []

    for config in CONFIGS:
        print(
            "[Week7-Day2] Reranking setting={}, N={}, M={}, compression={} ...".format(
                config["setting"],
                config["N"],
                config["M"],
                config["compression"],
        ...
        summary = rerank_for_config(config)
        summaries.append(summary)

        print(
            "[Week7-Day2] setting={} N={} M={} done. queries={}, total_candidates={}, total_scored={}, total_written={}, avg_tokens_after={:.4f}, avg_latency_ms={:.6f}".format(
                summary["setting"],
                summary["N"],
                summary["M"],
                summary["query_count"],
    ...
    print("")
    print("========== Week7 Day2 BM25 + Budgeted MV Completed ==========")
    print("Summary CSV: {}".format(summary_csv))
    print("Summary JSON: {}".format(summary_json))

    for s in summaries:
        print("")
        print("setting={} N={} M={} compression={}".format(s["setting"], s["N"], s["M"], s["compression"]))
        ...
        print("  Avg page tokens before: {:.4f}".format(s["avg_page_tokens_before"]))
        print("  Avg page tokens after: {:.4f}".format(s["avg_page_tokens_after"]))
        print("  Token keep ratio: {:.6f}".format(s["token_keep_ratio"]))
